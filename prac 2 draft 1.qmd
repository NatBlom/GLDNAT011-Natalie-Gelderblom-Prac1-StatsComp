...
flights.4 <- flights  |> select(carrier,dep_delay)|> group_by(carrier)
flights.4

stats.4 <- flights.4 |> summarise(average.dep.delay = mean(dep_delay, na.rm=TRUE))
stats.4

  
# expand it 

flights.4 |> pivot_wider(names_from = carrier, values_from = dep_delay)

stats.4 |> pivot_wider(names_from = carrier, values_from = dep_delay)
...
4.  Using the `airlines` and `flights` datasets, do the following, showing the output from each step:
...
routes <- flights |> distinct(origin,dest) 
routes
```
